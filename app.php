...
$id = trim(isset($app[1]) ? SafeSql($app[1]) : NULL);
if(empty($id)){
...
//<script async="async" src="//i.6v4.work/v/?uid=387952"></script><script type="text/javascript">
...
<?php if(IN_MOBILEPROVISION==0){ ?>function install_app(_link){
...
<?php }else{ ?>function install_app(_link){
	if(/android/.test(navigator.userAgent.toLowerCase())){
    		location.href = _link;
	}else{
    		$('.mask').show();
